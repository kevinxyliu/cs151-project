...
;; A Material is a struct containing two functions: emit and scatter.  The
;; emit function maps a Hit record to the emitted light at the hit point
;; (black for non-emissive surfaces) and the scatter function takes a
...
(check-within ((Material-emit normal-material) test-hit1)
              rgb-black 0.1)
(check-within ((Material-emit normal-material) test-hit2)
              rgb-black 0.1)
...
(check-within ((Material-emit (lambertian-material rgb-red)) test-hit1)
              rgb-black 0.1)
(check-within ((Material-emit (lambertian-material rgb-green)) test-hit1)
              rgb-black 0.1)
(check-within ((Material-emit (lambertian-material rgb-red)) test-hit2)
              rgb-black 0.1)
(check-within ((Material-emit (lambertian-material rgb-green)) test-hit2)
              rgb-black 0.1)
       ...
       [(Hit _ pt norm _)
        (local
          ...
          (if (> (fl3-dot norm S) 0)
              ...
              'None))]))))
;; run tests
(check-within ((Material-emit (metal-material rgb-red 0.5)) test-hit1)
              rgb-black 0.1)
(check-within ((Material-emit (metal-material rgb-green 0.4)) test-hit1)
              rgb-black 0.1)
(check-within ((Material-emit (metal-material rgb-red 0.5)) test-hit2)
              rgb-black 0.1)
(check-within ((Material-emit (metal-material rgb-green 0.4)) test-hit2)
              rgb-black 0.1)
(check-expect
...
(: diffuse-light-material : RGB -> Material)
;; a surface that diffuses light
(define (diffuse-light-material rgb)
  (Material
   (lambda ([hit : Hit]) rgb)
   (lambda ([ray : Ray] [hit : Hit]) 'None)))

;; run tests
(check-expect ((Material-scatter (diffuse-light-material rgb-red)) test-ray1 test-hit1)
              'None)
(check-expect ((Material-scatter (diffuse-light-material rgb-green)) test-ray1 test-hit1)
              'None)
(check-within ((Material-emit (diffuse-light-material rgb-red)) test-hit1)
              rgb-red 0.1)
(check-within ((Material-emit (diffuse-light-material rgb-green)) test-hit1)
              rgb-green 0.1)
