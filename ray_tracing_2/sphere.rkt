   ...
   (lambda ([ray : Ray] [min-max-t : Interval])
     (local
       ...
       (if (< d 0.0)
           'Miss
           (local
             {(define t : Float (/ (- (- b) (fl-sqrt d)) 2.0))
              (define Q : Float3 (ray-point-at ray t))
...
(check-expect
 (object-hit? (make-sphere (Float3 1.0 3.0 5.0) 1.0
              ...
              (make-ray fl3-zero (Float3 1.0 1.0 1.0)) (Pair 0.0 +inf.0)) 'Miss)
(check-within
 (Hit-t
  (test-helper
   (object-hit? (make-sphere (Float3 0.0 0.0 -2.0) 0.5
                             (flat-material (RGB 1.0 0.0 0.0)))
                (make-ray fl3-zero (Float3 0.0 0.0 -2.0)) (Pair 0.0 +inf.0))))
 1.5 0.1)
(check-within
 (Hit-pt
  (test-helper
